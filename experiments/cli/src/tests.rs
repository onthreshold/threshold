...
fn decrypt_private_key(
    ...
    let nonce_bytes = BASE64.decode(&params.iv_b64)
        .map_err(|e| KeygenError::Encryption(e.to_string()))?;
    let nonce = Nonce::from_slice(&nonce_bytes);
    
    let cipher = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(&key));
    
    let ciphertext = BASE64.decode(encrypted_key)
        .map_err(|e| KeygenError::Encryption(e.to_string()))?;
...
fn test_invalid_directory() {
    ...
    let result = generate_test_keypair(Some(PathBuf::from("/nonexistent/path/identity.key")), "test_password123");
